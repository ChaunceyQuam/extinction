...
#*flow control logic? to rturn NA for miss data
#  possibly
extinc_year
...
bin_year <- function(x) {
  ...
  x = as.integer(x)
    case_when(
      rlang::is_empty(x) ~ 1900,
      between(x, 1500, 1600) ~ 1500,
      between(x, 1600, 1700) ~ 1600,
      between(x, 1700, 1800) ~ 1700,
      between(x, 1800, 1900) ~ 1800,
      between(x, 1900, 2020) ~ 1900,
      !between(x, 1500, 2020) ~ 0)
      unique() %>%
      max()
}
```
...
#use merge to combine both tables
```{r}

```

We need to create a table that has rows of species that went extinct and year the species went extinct
```{r}
joined <- left_join(all_year, all_species, by = "scientific_name")
rbind()
