...
According to the paper by Ceballos on species extinction, he selected both extinct species and extinct in the wild species, as categorized by the IUCN. We will try to reproduce his result by selecting the same categories.
```{r cache=TRUE}
...
all_extinct_total <- data.frame(name = c("birds", "mammals", "vertebrate", "Other vertebrates"), 
                                  value_sum = c(nrow(birds_total), nrow(mammals_total), nrow(vertebrate_total),nrow(all_other_total)))
all_extinct_total                                

master_ex_table <- rbind(birdies, mammalies, vert, others) %>%
  left_join(all_extinct_total) %>%
  mutate(fraction_extinct = count/ value_sum)
master_ex_table
```
```{r cache=TRUE}
all_species_count <- all_results  %>% map_dfr(get_result)
```

This is the total number of species evaluated by the ICUN
```{r}
nrow(all_species_count)
```
Lets create a master table

```{r}
birds_total <- all_species_count %>% filter(class == "AVES")
mammals_total <- all_species_count %>% filter(class == "MAMMALIA")
vertebrate_total <- all_species_count %>% filter(class %in% verte)
all_other_total <- all_species_count %>% filter(!class %in% verte)
