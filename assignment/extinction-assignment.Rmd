...
all_extinct_mammal_bird <- all_species %>% filter(category=="EX") %>% filter(class=="MAMMALIA") %>% filter(class=="AVES") %>% pull(scientific_name)
...
get_result <- function(x){
  x %>% content() %>% getElement("result") %>%
    map_dfr(safely(row_to_tibble_1, NA))
}
...
good_narrative %>% map_dfr(get_result)
...
```{r}
  result <-content(responses)
  text <- result$result[[1]]$rationale %>%
    stringr::str_extract_all("\\d\\d\\d\\d")
}

request_API_narrative ()

```
```{r}
paste0(base, narrative, name, query, token)
```

#writing a function to get narrative 
```{r}

```

```{r}
result <- content(dummy)
text <- result$result[[1]]$rationale
text
```
```{r}
